...
export function QuoteForm({ onSubmit, defaultValues }: QuoteFormProps) {
  ...
  const totals = calculateTotals(watchedItems, watchedDiscount, watchedShipping)
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
      ...
      <section className={SECTION}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input {...register('number')} className={errors.number ? INPUT_ERR : INPUT} />
            {errors.number && <p className="text-xs text-red-500 mt-1">{errors.number.message}</p>}
          </div>
          <div>
            <label className={LABEL}>Fecha</label>
            <input {...register('date')} type="date" className={errors.date ? INPUT_ERR : INPUT} />
      ...
      <section className={SECTION}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input {...register('companyName')} placeholder="Nombre de la empresa" className={INPUT} />
          </div>
        </div>
      </section>
      ...
      <section className={SECTION}>
        ...
        <button
          type="button"
          onClick={() => append({ quantity: '1', description: '', unitPrice: '' })}
          className="mt-4 text-sm text-blue-600 hover:text-blue-800 font-medium flex items-center gap-1"
        >
          <span className="text-lg leading-none">+</span> Agregar producto / servicio
        </button>
        ...
        <div className="flex flex-col sm:flex-row gap-5">
          <div className="sm:w-44 space-y-4">
            <div>
              ...
              <input
                {...register('discount', { setValueAs: (v) => (v === '' || isNaN(Number(v))) ? 0 : Number(v) })}
                type="number"
                min="0"
                step="0.01"
                defaultValue={0}
            ...
            <div>
              ...
              <input
                {...register('shipping', { setValueAs: (v) => (v === '' || isNaN(Number(v))) ? 0 : Number(v) })}
                type="number"
                min="0"
                step="0.01"
                defaultValue={0}
          ...
          <div className="flex-1 bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
            ...
            {totals.discount > 0 && (
              <div className="flex justify-between">
                ...
                <span className="font-medium text-red-500">−{mxn(totals.discount)}</span>
              </div>
            )}
